...
router.post('/vendas', async (req, res) => {
    ...
    try {
        ...
        // 1. Criar o pagamento no Mercado Pago PRIMEIRO para pegar o ID oficial da transacao
        const mpResponse = await payment.create({
            body: {
                transaction_amount: Number(total.toFixed(2)),
                description: 'Compra de Livro - E-Commerce',
        ...
        // 2. Montar array para salvar as vendas interligadas pelo ID do MP
        for (const item of items) {
                ...
                if (existingBook && existingBook.id) {
                    ...
                    const { data: newBook, error: insertErr } = await supabase
                        ...
                        .insert([{
                            ...
                            titulo: item.title,
                            autor: item.author,
                            preco: Math.round(Number(item.price)),
                            capa_url: item.image,
                    ...
                    if (newBook && newBook.id) {
                        ...
                        throw new Error("Não foi possível resolver o UUID do livro externo: " + item.title + " | Erro do banco: " + (insertErr ? insertErr.message : "Sem dados"));
                    }
                }
            }
            ...
            vendasData.push({
                ...
                id_pagamento_mp: paymentId
            });
        }

        // Insere as vendas "pendentes" 
        ...
        // NAO INSERE NA BIBLIOTECA AINDA.
        // O livro de fato só é seu, quando o Mercadopago gritar que ta pago!

        res.json({ 
            success: true, 
            message: "Pedido aguardando pagamento PIX!",
            paymentId: paymentId,
        ...
        res.status(500).json({ success: false, message: error.message || "Erro ao processar checkout" });
    }
});

// GET /api/pagamento/status/:id - Polling/Webhook de consulta do status do pagamento
router.get('/pagamento/status/:id', async (req, res) => {
    const paymentId = req.params.id;

    try {
        // Consultar status real na API do Mercado Pago
        ...
        if (mpStatus.status === 'approved') {
            ...
            const { data: vendasDb } = await supabase
                .from('vendas')
                .select('*')
                .eq('id_pagamento_mp', paymentId);
            
            // Prevenir de colocar na biblioteca varias vezes (já estando aprovado)
            if (vendasDb && vendasDb.length > 0 && vendasDb[0].status_pagamento !== 'aprovado') {
                // Atualizar vendas -> aprovado
                await supabase
                    .from('vendas')
                    .update({ status_pagamento: 'aprovado' })
                    .eq('id_pagamento_mp', paymentId);
                
                // Mover os itens pagos definitivamente pra biblioteca do perfil
                const bibliotecaData = vendasDb.filter(v => v.perfil_id).map(v => ({
            ...
            res.json({ success: true, status: mpStatus.status }); 
        }

    } catch (error) {
        console.error('Erro ao buscar status do mercadopago:', error);
